...
CREATE TABLE IF NOT EXISTS challenges (
  ...
  title text NOT NULL,
  game_type text NOT NULL CHECK (game_type IN ('FIFA', 'NBA2K', 'OTHER')),
  platform text NOT NULL CHECK (platform IN ('PS5', 'XBOX', 'PC')),
  rules text,
  required_evidence text CHECK (required_evidence IN ('SCREENSHOT', 'VIDEO', 'BOTH')),
...
DECLARE
  v_challenge record;
  v_challenger_wallet_id uuid;
  v_challenged_wallet_id uuid;
BEGIN
